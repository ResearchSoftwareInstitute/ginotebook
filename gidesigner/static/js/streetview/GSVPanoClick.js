...
GSVPANO.PanoLoader = function (parameters) {
	...
	this.composeFromTile = function (x, y, texture) {
		...
		//this.OilPntEff(_ctx[ py * _wc + px ], _canvas[ py * _wc + px], x, y, 3, 30);
		this.progress();
	}; 

	this.OilPntEff = function(curr_ctx, canvas, canvas_left, canvas_top, radius, intensity)
	{
    var width = 512,
        height = 512,
        imgData = curr_ctx.getImageData(canvas_left, canvas_top, width, height),
        pixData = imgData.data,
	//	lowResoluData = new Array(width*height),
        pixelIntensityCount = [];
    ...
    for (var y = 0; y < height; y++) {
        ...
        for (var x = 0; x < width; x++) {
            var idx = (y * width + x) * 4,
             //   r = lowResoluData[idx],
               // g = lowResoluData[idx + 1],
               // b = lowResoluData[idx + 2],
			   r = pixData[idx],
               g = pixData[idx + 1],
               b = pixData[idx + 2],
                avg = (r + g + b) / 3;
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            destPixData[dIdx] = Math.max(~~ (pixelIntensityCount[0].r / curMax), rgbLUT[y ][x].r);
            destPixData[dIdx + 1] = Math.max(~~ (pixelIntensityCount[0].g / curMax),rgbLUT[y][x].g);
            destPixData[dIdx + 2] = Math.max(~~ (pixelIntensityCount[0].b / curMax),rgbLUT[y][x].b);
            destPixData[dIdx + 3] = 255;
			
        }
    }

    curr_ctx.putImageData(destImageData, canvas_left, canvas_top);
	...
	this.loadPano = function( location, id ) {
		...
		_panoClient.getPanoramaById( id, function (result, status) {
			if (status === google.maps.StreetViewStatus.OK) {
				...
				cur_available_streetview_lat = result.location.latLng.G;
				cur_available_streetview_lng = result.location.latLng.K;
				cur_panoId = _panoId;
				self.location = location;
				self.composePanorama();
				(new get_latlng()).setDepthMap();//set depthmap when loading
			} else {
